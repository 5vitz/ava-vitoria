...
def generate_slide8():
    ...
    output_path = os.path.join(project_root, "public/imagens/SLIDES/08 - slide_crescimento.jpg")
    preview_path = os.path.join(project_root, "public/imagens/slide_crescimento_preview.jpg")
    ...
    title_text = "E S T R A T É G I A  D E  C R E S C I M E N T O"
    subtitle_text = "Como escalaremos a comunidade local de Vitória para o território nacional."
    ...
    columns = [
        {
            "x": x_margin,
            "title": "1. DROPS LIMITADOS",
            "desc": "Lançamento de coleções temáticas em lotes estritos e exclusivos. O gatilho da escassez acelera a decisão de compra imediata e transforma as roupas em itens colecionáveis."
        },
        {
            "x": x_margin + col_w + col_gap,
            "title": "2. EMBAIXADORES DE NICHO",
            "desc": "Parcerias orgânicas com skatistas, artistas de grafite e formadores de opinião em capitais estratégicas (SP, RJ, BA). Exportamos nossa atitude local através de conexões genuínas."
        },
        {
            "x": x_margin + 2 * (col_w + col_gap),
            "title": "3. MÍDIA INTELIGENTE (SEO/GEO)",
            "desc": "Otimização do site e da presença digital da marca para motores tradicionais (SEO) e motores de resposta por Inteligência Artificial (GEO), garantindo que a AVA seja recomendada por assistentes como Gemini e ChatGPT."
        }
    ]
    ...
    # 7. Create a 1:1 close-up preview of Column 3 (SEO/GEO)
    crop_w = 380
    crop_h = 320
    crop_box = (
